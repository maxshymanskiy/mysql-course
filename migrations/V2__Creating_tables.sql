...
CREATE TABLE IF NOT EXISTS vehicles (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_vehicles_to_clients FOREIGN KEY (client_id)
        REFERENCES clients (client_id)
        ON DELETE CASCADE 
		ON UPDATE CASCADE,
    CONSTRAINT fk_vehicles_to_makes FOREIGN KEY (make_id)
        REFERENCES vehicle_makes (make_id)
        ON DELETE CASCADE 
		ON UPDATE CASCADE,
    CONSTRAINT fk_vehicles_to_types FOREIGN KEY (vehicle_type_id)
...
CREATE TABLE IF NOT EXISTS employees (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_employees_to_positions FOREIGN KEY (position_id)
        REFERENCES positions (position_id)
        ON DELETE CASCADE
		ON UPDATE CASCADE,
    CONSTRAINT chk_employees_pay_rate
		CHECK (pay_rate >= 0),
    CONSTRAINT chk_employees_commission 
...
CREATE TABLE IF NOT EXISTS services (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT chk_services_hours 
		CHECK (estimated_hours > 0),
    CONSTRAINT chk_services_rate 
		CHECK (hourly_rate >= 0)
);
...
CREATE TABLE IF NOT EXISTS repair_orders (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_repair_orders_to_clients FOREIGN KEY (client_id)
        REFERENCES clients (client_id)
        ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT fk_repair_orders_to_vehicles FOREIGN KEY (vehicle_id)
        REFERENCES vehicles (vehicle_id)
        ON DELETE CASCADE 
		ON UPDATE CASCADE,
    CONSTRAINT fk_repair_orders_to_mechanics FOREIGN KEY (assigned_mechanic_id)
        REFERENCES employees (employee_id)
        ON DELETE CASCADE 
		ON UPDATE CASCADE,
    CONSTRAINT chk_repair_orders_paid_amount 
		CHECK (paid_amount >= 0),
    CONSTRAINT chk_repair_orders_total_amount 
		CHECK (total_amount >= 0),
    CONSTRAINT chk_repair_orders_payment_valid 
		CHECK (paid_amount <= COALESCE(total_amount, paid_amount))
);
...
CREATE TABLE IF NOT EXISTS repair_order_parts (
    ...
    CONSTRAINT fk_repair_order_parts_to_orders FOREIGN KEY (order_id)
        REFERENCES repair_orders (order_id)
        ON DELETE CASCADE 
		ON UPDATE CASCADE,
    CONSTRAINT fk_repair_order_parts_to_parts FOREIGN KEY (part_id)
        REFERENCES parts (part_id)
        ON DELETE CASCADE 
		ON UPDATE CASCADE,
    CONSTRAINT fk_repair_order_parts_to_mechanics FOREIGN KEY (mechanic_id)
        REFERENCES employees (employee_id)
        ON DELETE SET NULL 
		ON UPDATE CASCADE,
    CONSTRAINT chk_repair_order_parts_quantity 
		CHECK (quantity_used > 0),
    CONSTRAINT chk_repair_order_parts_price 
		CHECK (price_applied >= 0)
);
...
CREATE TABLE IF NOT EXISTS repair_order_services (
    ...
    CONSTRAINT fk_repair_order_services_to_orders FOREIGN KEY (order_id)
        REFERENCES repair_orders (order_id)
        ON DELETE CASCADE 
		ON UPDATE CASCADE,
    CONSTRAINT fk_repair_order_services_to_services FOREIGN KEY (service_id)
        REFERENCES services (service_id)
        ON DELETE CASCADE 
		ON UPDATE CASCADE,
    CONSTRAINT fk_repair_order_services_to_mechanics FOREIGN KEY (mechanic_id)
        REFERENCES employees (employee_id)
        ON DELETE CASCADE 
		ON UPDATE CASCADE,
    CONSTRAINT chk_repair_order_services_hours 
		CHECK (actual_hours >= 0),
    CONSTRAINT chk_repair_order_services_price 
